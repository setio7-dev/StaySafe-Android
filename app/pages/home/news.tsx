...
export default function News() {
  const { news, setSearch, search, fetchNews } = useNewsHook();
  ...
  return (
    <CustomSafeAreaView>
      <ScrollView className='flex flex-col' contentContainerStyle={{ paddingBottom: 40 }}>
        <PrimaryGradient className='flex flex-col px-6 pt-10' roundedBottom={12}>
          ...
          <SearchPrimary value={search} onChange={(e) => setSearch(e)} className='translate-y-6 elevation-md z-20' />
        </PrimaryGradient>
        <View className='flex flex-col px-6 pt-14 gap-6'>
          {news.length > 0 ? (
            news.map((item, index) => (
