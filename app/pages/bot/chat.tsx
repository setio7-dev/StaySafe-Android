...
export default function Chat() {
    const { fetchConversation, conversation, user, message, setMessage, handleMessage, typingMessageId, typingText } = useSafeTalkHook();    
  ...
  return (
    <CustomSafeAreaView>
                        ...
                        <View key={index} className={`flex ${item.role === "user" ? 'flex-row' : 'flex-row-reverse'} gap-2 flex-1`}>
                            <View className={`flex ${item.role === "user" ? 'flex-row' : 'flex-row-reverse'} flex-1`}>
                                <PrimaryGradient className='p-4 flex-1' roundedBottom={12} roundedTop={12}>
                                    {typingMessageId === item.id ? (
                                        <Text className='font-poppins_medium text-justify text-white text-[12px]'>{typingText.replace(/\*\*/g, "").trim()}</Text>
                                    ) : (
        ...
        <View className='mb-4 bg-white px-6 py-4'>
